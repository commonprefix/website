...
***_2026-8-12 update: Added [link](#bonus-proving-closed-source-code-is-true-to-spec-with-zero-knowledge) to zkPi paper_***
...
What if you want to keep code closed-source but also prove it upholds a certain spec? Dan Boneh proposed ([1](https://zeroknowledge.fm/podcast/400), [2](https://eprint.iacr.org/2024/267)) using [Zero-Knowledge proofs](https://en.wikipedia.org/wiki/Zero-knowledge_proof) for proving in zero knowledge the existence of a proof of adherence to some spec. Complicated, I know, and very much theoretical for now, but it would definitely integrate nicely with the automated assurance system described above. The developer publishes the compiled binary, the spec, and the zero-knowledge proof that shows the binary has the spec's properties. The developer would need a [zero-knowledge VM](https://docs.succinct.xyz/docs/sp1/what-is-a-zkvm) that internally runs the proof checker. The assurance pipeline would need a trusted zero-knowledge verifier to check the proof.
